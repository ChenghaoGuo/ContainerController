...
open class TableAdapterView: UITableView {
  ...
  public func unsafeAdd(item: TableAdapterItem) {
    ...
    if item.useNib {
      ...
      registerIfNeeded(for: item)
    }
  }

  public func registerIfNeeded(for item: TableAdapterItem) {
    self.register(item.cellClass, forCellReuseIdentifier: item.cellReuseIdentifier)
  }
